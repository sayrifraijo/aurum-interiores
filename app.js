...
(function () {
  ...
  var BUYABLE = [].concat(D.products || [], D.lighting || [], D.hardware || [], D.equipment || []);
  ...
  function renderHero() {
    ...
    return '<section class="hero" id="top">' +
      '<div class="wrap hero__masthead">' +
        '<image-slot id="aurum-logo" class="hero__logo" fit="contain" position="left center" placeholder="Logo Aurum"></image-slot>' +
      ...
      '<figure class="hero__cover">' +
        '<image-slot id="hero-render" class="hero__render" fit="cover" placeholder="Arrastra aquí el render principal del proyecto"></image-slot>' +
  ...
  function renderResumen() {
    ...
    return '<section class="section" id="resumen"><div class="wrap">' +
      ...
      '<div class="concept"><div><p class="concept__statement">' + esc(c.statement) + '</p>' + body + '</div><aside class="concept__aside"><span class="label">El estilo</span><h4>' + esc(c.style) + '</h4><p>' + esc(c.definition) + '</p></aside></div>' +
  ...
  function renderPanel() {
    ...
    var tiles = md.tiles.map(function (t) {
      var inner = t.img ? '<img src="' + esc(t.img) + '" alt="' + esc(t.label) + '">' : '<div class="tile__ph"><span class="label">' + esc(t.note) + '</span><span class="ph-name">' + esc(t.label) + '</span></div>';
    ...
    return '<section class="section" id="panel"><div class="wrap">' +
      '<div class="section__head"><div><div class="section__kicker"><span class="num">' + esc(md.sectionCode) + '</span><span class="label">Panel Conceptual</span></div><h2 class="section__title">Atmósfera & materiales</h2></div><p class="section__note">La paleta y los materiales guían toda la selección. Arrastra tus renders en cada espacio.</p></div>' +
  ...
  function productCard(p) {
    ...
    var media = p.img ? '<img src="' + esc(p.img) + '" alt="' + esc(p.name) + '">' : '<div class="card__ph"><span class="label">Foto de producto</span></div>';
  ...
  function finishTable(items, showSpace) {
    ...
    return '<div class="table-wrap"><table class="ltable ltable--finish"><thead><tr>' +
      '<th>Tipo</th><th>Material · Acabado</th><th>Proveedor · SKU</th>' + (showSpace ? '<th>Espacio</th>' : '') + '<th>Superficie</th>' +
      '</tr></thead><tbody>' + rows + '</tbody></table></div>';
  }
  ...
  function carpentryCards(items) {
    ...
    var cards = items.map(function (c) {
      var media = c.img ? '<img src="' + esc(c.img) + '" alt="' + esc(c.name) + '">' : '<div class="card__ph carp__ph"><span class="label">Plano de despiece</span></div>';
      return '<article class="carp-card"><div class="carp-card__media">' + media + '</div>' +
        '<div class="carp-card__body"><span class="label">' + esc(c.space) + '</span><h4 class="carp-card__name">' + esc(c.name) + '</h4>' +
        '<dl class="spec"><dt>Medidas</dt><dd class="tnum">' + esc(c.dims) + '</dd><dt>Materiales</dt><dd>' + esc(c.materials) + '</dd><dt>Acabado</dt><dd>' + esc(c.finish) + '</dd></dl>' +
        '<p class="carp-card__note">' + esc(c.note) + '</p></div></article>';
    }).join("");
  ...
  function renderRenders(sp) {
    ...
    if (rs.length) {
      tiles = rs.map(function (r) {
        var inner = r.img ? '<img src="' + esc(r.img) + '" alt="' + esc(r.label) + '">' : '<div class="tile__ph"><span class="label">' + esc(r.note) + '</span><span class="ph-name">' + esc(r.label) + '</span></div>';
  ...
  function renderProgramaBody() {
    ...
    if (view === "espacio") {
      ...
      var blocks = [
        ...
        { key: "Acabados", code: "03.F", label: "Esquema de Acabados", count: D.finishes.length, html: finishTable(D.finishes, true), note: "Informativo: material, acabado, proveedor y superficie por elemento." }
  ...
  function planosHtml(items) {
    return '<div class="planos-grid">' + items.map(function (p) {
      var inner = p.img ? '<img src="' + esc(p.img) + '" alt="' + esc(p.title) + '">' : '<div class="plano__ph"><span class="label">' + esc(p.note) + '</span><span class="ph-name">' + esc(p.title) + '</span></div>';
  ...
  function buyablePool(c) {
    ...
    return []; // Carpintería y Acabados son informativos (no comprables)
  }
  ...
  function boot() {
    ...
    document.getElementById("programaBody").addEventListener("click", function (e) {
      ...
      if (card && e.target.closest(".add-toggle")) { toggleSel(card.getAttribute("data-id")); }
    });
